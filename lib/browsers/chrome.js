...
const sleep = (durationMs) => new Promise((resolve) => setTimeout(() => {
    resolve()
}, durationMs));
...
chrome.connect = function () {
    return new Promise((resolve, reject) => {
        ...
        let connect = () => {
            CDP.Version({
                host: this.options.browserDebuggingHost,
                port: this.options.browserDebuggingPort
            }).then((info) => {
...
chrome.openTab = function (options) {
    return new Promise((resolve, reject) => {
        ...
        const connectToBrowser = async (target, port) => {
            ...
            for (; ;) {
                try {
                    return await CDP({target, host: this.options.browserDebuggingHost, port});
                } catch (err) {
